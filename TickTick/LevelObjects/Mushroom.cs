...
class Mushroom : SpriteGameObject
{
    ...
    public Mushroom(Level level, Vector2 startPosition) : base("Sprites/LevelObjects/mushroom", TickTick.Depth_LevelObjects)
    {
    ...
    protected void randomPower()
    {
        ...
        if (Visible && level.Player.CanCollideWithObjects && HasPixelPreciseCollision(level.Player))
        {
            ...
            {
                case 0:
                    level.Player.walkingSpeedAdjustment -= 100;
                    break;
                case 1:
            ...
            Visible = false;
            ExtendedGame.AssetManager.PlaySoundEffect("Sounds/snd_mushroom");
        }
